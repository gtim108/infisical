...
type TLicenseServiceFactoryDep = {
  ...
  licenseDAL: Pick<TLicenseDALFactory, "countOfOrgMembers" | "countOrgUsersAndIdentities">;
...
export const licenseServiceFactory = ({
  ...
  // live usage counts, writes it to the plan cache, and returns it. Throws on any failure — it never
  // persists the free-tier fallback itself, so callers control that decision.
  const fetchAndCacheCloudPlan = async (orgId: string): Promise<TFeatureSet> => {
    ...
    currentPlan.membersUsed = await licenseDAL.countOfOrgMembers(rootOrgId);
    currentPlan.identitiesUsed = await licenseDAL.countOrgUsersAndIdentities(rootOrgId);
