...
export const proxiedServiceDALFactory = (db: TDbClient) => {
  ...
  // Only advance lastUsedAt; a stale/out-of-order report matches no row and skips the write.
  const stampLastUsed = async (serviceId: string, usedAt: Date, tx?: Knex) => {
    await (tx || db)(TableName.ProxiedService)
      .where({ id: serviceId })
      .andWhere((qb) => {
        void qb.whereNull("lastUsedAt").orWhere("lastUsedAt", "<", usedAt);
      })
      .update({ lastUsedAt: usedAt });
  };
